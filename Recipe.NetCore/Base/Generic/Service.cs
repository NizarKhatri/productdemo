...
namespace Recipe.NetCore.Base.Generic
{
     ...
     where TKey : IEquatable<TKey>
    {
        ...
        protected async Task<TEntity> Update(TDTO dtoObject)
        {
            ...
            if (dbEntity != null)
            {
                CopyAllTo(_mapper.Map<TEntity>(dtoObject), dbEntity);
            }
        ...
        public void CopyAllTo(TEntity source, TEntity target)
        {
            var type = typeof(TEntity);
            foreach (var sourceProperty in type.GetProperties())
            {
                var targetProperty = type.GetProperty(sourceProperty.Name);
                if (sourceProperty.Name != "CreatedBy" && sourceProperty.Name != "CreatedOn" && sourceProperty.Name != "Id")
                {
